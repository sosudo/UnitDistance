...
-- golodShafarevich_criterion is imported from STANDARD_AXIOMS.

/-!
## Frattini-Burnside theorem (Proposition A.8)

For a finitely generated pro-p group G:
- d(G) = dim_{F_p}(G/Phi(G))
- If g_1,...,g_k in Phi(G) and N is their closed normal closure, then
...
/-- **Frattini-Burnside** (Proposition A.8): For a finitely generated pro-p group G,
    if g_1,...,g_k in Phi(G) generate a closed normal subgroup N, then d(G/N) = d(G) and
    r(G/N) <= r(G) + k.
...
A nontrivial finitely generated pro-p group with r <= d^2/4 is infinite.

Uses `golodShafarevich_criterion` from STANDARD_AXIOMS.lean (is_assumption: true).
-/

/-- **Golod-Shafarevich** (Proposition A.9): A nontrivial finitely generated pro-p
    group G with r(G) <= d(G)^2/4 is infinite.
    Uses the external axiom `golodShafarevich_criterion` from STANDARD_AXIOMS.lean.
    is_assumption: true (Golod-Shafarevich is a permitted axiom per UNITY.md invariant 6). -/
